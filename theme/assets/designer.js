...
const doAlerts = false;
doAlerts && alert("imported this module");
class DesignerComponent {
    ...
    static init(selector, className = "anvil-extras-designer") {
        const doInit = () => {
            this.loading = true;
            doAlerts && alert(`about to load ${this.name}`);
            $(() => {
                ...
                this.loading = false;
                doAlerts && alert(`finished loading ${this.name}`);
            });
        };
        ...
        if (!this.loaded && !this.loading) {
            ...
            this.loading = true;
            doAlerts && alert(`loading for the the first time ${this.name} ${this.loading} ${this.loaded}`);
            ...
            doAlerts && alert(`done init load - doing another load of ${this.name} ${this.loading} ${this.loaded}`);
            doInit();
        } else {
            doAlerts && alert(`already loading ${this.name} ${this.loading} ${this.loaded}`);
        }
    }
    ...
    static setup(selector, className) {
        for (let el of document.querySelectorAll(selector)) {
            ...
            if (container.classList.contains(className)) {
                doAlerts && alert(`tried to set up but was already setup ${this.name}`);
                return;
            }
            ...
            doAlerts && alert(`setting up ${self}`);
            if (props) {
    ...
    update(props, propName = null, val = null) {}

    setProp(propName, val) {}

    updateSpacing({ spacing_above, spacing_below }) {
        const stale_spacing = Array.prototype.filter.call(this.domNode.classList, (x) =>
    ...
    getColorRGB(color, _default) {
        ...
        if (color && color.startsWith("#")) {
            ...
            return color.slice(4, color.lengh - 1);
        }
        return color;
    }
...
export class DesignerTabs extends DesignerComponent {
    ...
    constructor(container, pyComponent, el) {
        super(container, pyComponent, el);
        this.tabs = el;
...
export class DesignerSwitch extends DesignerComponent {
    static defaults = {};
    static css = `.switch,.switch *{-webkit-tap-highlight-color:transparent;-webkit-user-select:none;-moz-user-select:none;-ms-user-select:none;user-select:none}
...
export class DesignerMultiSelectDropDown extends DesignerComponent {
    static defaults = { placeholder: "None Selected", enabled: true, visible: true, spacing_above: "small", spacing_below: "small" };
...
export class DesignerSlider extends DesignerComponent {
    static defaults = { start: [20, 80], connect: true, min: 0, max: 100, visible: true, enabled: true };
    ...
    getFormatter(formatspec) {
        ...
        formatspec = Sk.ffi.toPy(formatspec);
        const format = formatspec.tp$getattr(Sk.ffi.toPy("format"));
    ...
    update(props) {
        try {
            ...
            this.updateSpacing(props);
            this.updateVisible(props);
            props.enabled ? this.slider.removeAttribute("disabled") : this.slider.setAttribute("disabled", true);
